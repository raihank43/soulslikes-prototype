...
### Day 4.5 — Mutant moveset depth (Swipe, Jump Attack, combos, tells)

- **`EnemyAttackType` enum (Punch/Swipe/JumpAttack) in `EnemyAttack`.** `BeginAttack(EnemyAttackType)` routes to the matching animator trigger (`PunchTrigger`/`SwipeTrigger`/`JumpAttackTrigger`). `EnemyAttack.DurationFor(type)` returns the wall-clock swing length at AttackSpeed=1 (Punch 0.88, Swipe 2.0, JumpAttack 2.8) — the AI waits this long, NOT an animation event (see clip-length gotcha below).
- **`EnemyAI.AttackRoutine` (coroutine) owns the whole attack sequence**, replacing the old `EnterAttack`/`TickAttack` flag polling. Per swing it: picks an attack for the current range, applies a wind-up delay, sets `AttackSpeed`, sets the root-motion multiplier, fires the attack, waits `DurationFor()/AttackSpeed`, calls `attack.AttackComplete()`, then decides whether to chain. Cancelled by `EnterHurt`/`OnDied` via `StopCoroutine(attackRoutine)`.
- **Distance-banded selection (`PickAttackForRange`):** `punchRange=1.6`, `swipeRange=2.8`, `jumpAttackMinRange=3.5`, `jumpAttackMaxRange=6.0`. Close/mid bands use `WeightedPick` (weighted-random with the **last-used attack's weight halved** to suppress repeats). Returning `null` from the picker means "no attack for this range" → the AI stays in Chase to close the gap. This is how the far band throttles leaping.
- **JumpAttack has its own cooldown** (`jumpAttackCooldown=5s`, separate from `attackCooldown=1.2s`). While on cooldown the far band returns null (chase closer) instead of leaping. Without this, a player hovering at jump range gets leap-spammed because the generic cooldown is too short. Per-attack cooldowns are the pattern for any future "signature" move.
- **Adaptive lunge distance:** `MutantRootMotionForwarder.motionMultiplier` (NonSerialized, set per-swing by the AI) scales baked XZ root motion. Swipe is a flat `swipeMotionMultiplier=1.5`. JumpAttack is computed at strike-commit to land `jumpAttackLandClearance` (1m) short of the player: `mult = clamp((dist - clearance)/jumpAttackClipTravel, 1, jumpAttackMaxMultiplier)`. **Locked at commit**, so a player who moves after the leap starts can still dodge it (soulslike commitment).
- **`EnemyAttackMotionBehaviour` (StateMachineBehaviour) on Swipe + JumpAttack states** flips `applyRootMotion=true` on enter, restores on exit. Required because `MutantRootMotionForwarder` only forwards delta when `applyRootMotion` is true. Punch deliberately has NO such SMB — it's an in-place strike.
- **Combo chains:** `comboChance=0.5`, `comboMaxLength=2`. After a swing completes, the routine re-rolls and re-evaluates range — so a chain aborts cleanly if the player rolled out of all bands. `lastAttackType` biases the next pick away from a repeat and resets on `EnterReturning`.
- **Animator additions:** states `Swiping`, `JumpAttack` (tag=`Attacking`, iKOnFeet=false, `speedParameter=AttackSpeed`, `EnemyAttackMotionBehaviour` SMB). Params `SwipeTrigger`, `JumpAttackTrigger`, `AttackSpeed`(float, default 1). Punch also bound to `AttackSpeed`. Swipe→Locomotion `exitTime=0.40`, JumpAttack→Locomotion `exitTime=0.30` (low because the clips are length-inflated — see below).

### Gotchas hit on Day 4.5

- **Authoring animation events via the FBX importer inflates `clip.length` on every reimport.** Setting `ModelImporterClipAnimation.events` and calling `SaveAndReimport()` repeatedly caused `clip.length` to balloon (~2.6× per pass: Swipe 2.67s → 5.33s → 10.67s…) while the true take length stayed correct. Event times computed from `clip.length` then drifted past the visible swing AND past the state exit time (where they're silently dropped). **Fix: do NOT drive enemy attack completion from an `AttackComplete` animation event.** The AI coroutine waits an absolute `DurationFor(type)` instead. `EnableHitbox`/`DisableHitbox` still work because they sit at low clip fractions, well before exit time.
- **Inflated clip length makes `normalizedTime` thresholds unreliable across attacks.** Punch stays 1.1s but Swipe/JumpAttack inflate, so "wait until normalizedTime ≥ 0.8" waits wildly different real durations. Absolute-time waits are the robust path here.
- **A new attack state with baked translation needs the root-motion SMB OR it stays in place.** Adding Swipe/JumpAttack without `EnemyAttackMotionBehaviour` left them swinging on the spot — `applyRootMotion` defaults off and the forwarder drops the delta.
- **Per-roll attack selection ≠ rate limiting.** A 35% leap chance still spams because the AI re-rolls every `attackCooldown`. Throttle signature moves with a dedicated cooldown timer, not just a lower probability.

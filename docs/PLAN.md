...
- **Day 5 dodge — final = turn-and-roll hybrid (after a long playtest loop):** the Mixamo dodge clips have a body-vs-root-motion angle offset (~36–57°), so scripted-straight movement skated. Final approach uses the clips' **real root motion**: forward/back play a dive + rotate-to-aim (character turns into the roll); left/right play directional sidesteps keeping facing. Sped to `1.5×` with a softened landing transition. Known cosmetic: the dive lands in a crouch (mostly hidden by the transition); a tuck-roll clip would remove it. Open future ideas (user raised): edit clip root-motion in Blender to unlock true keep-facing strafe-dodges; screen new clips by measuring `averageSpeed` angle on import.

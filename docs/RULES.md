...
- **Rapid play→stop cycling can silently flip `EnterPlayModeOptions` (Disable Domain Reload) ON** in `ProjectSettings/EditorSettings.asset`. Keep it `None` (full domain reload) — otherwise static fields / event subscriptions leak between play sessions and cause confusing bugs. If `m_EnterPlayModeOptions: 1` shows up in a diff unexpectedly, revert it; don't commit it.
...
- **Mixamo clips can have a body-vs-root-motion angle offset** (the dodge clips travel ~36–57° off their body facing). Screen a new travel clip with **Tools ▸ Soulslike ▸ Measure Clip Travel** (select the FBX, or it scans `Models/dodge`) — it prints the `averageSpeed` angle, i.e. the `clipTravelAngle` to plug into turn-and-roll. If offset, scripted-straight movement will skate — use the clip's real root motion (+ rotate to aim) or fix the clip in Blender. Most clips (locomotion, attacks) are fine, so swapping in a clip you *like* and measuring it is the normal flow.

...
_Last updated: 2026-06-21 (1984e55) — recency signal, not a correctness guarantee. If the code has moved past this, trust the code._
...
- **The dive clip lands in a crouch**, so the `Dodge→Locomotion` transition pops to standing. Softened with `speed 1.5` + exit `0.9` + `0.30s` fixed blend (the crouch is most of the dive's tail, so this is a soften, not a cure); a tuck-roll clip that ends standing removes it fully. **`Tools ▸ Soulslike ▸ Measure Clip Travel`** prints any clip's travel angle so a replacement can be aimed (`clipTravelAngle`) with no skating.

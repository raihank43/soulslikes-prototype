...
- `Scripts/Player/RootMotionForwarder.cs` — forwards attack-clip root motion to the Rigidbody (Y stripped).
- Hierarchy: `Player` (Rigidbody, CapsuleCollider, controller/combat/stamina/health/lock-on) → child `Y Bot` (Animator with `PlayerLocomotion.controller`, **Apply Root Motion off**).
